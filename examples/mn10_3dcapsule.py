...
from torch_geometric.nn.modules import SplineConv, MLPConv, CapsLayer, \
    PoolingByAgreement, CapsLayerQuat, PoolingByAgreementQuat  # noqa
...
class Net(nn.Module):
    def __init__(self):
        ...
        self.conv1 = SplineConv(1, 72, dim=3, kernel_size=5)
        self.caps0 = CapsLayer(num_caps_in=1, num_caps_out=6,
                                        channels_in=4, channels_out=4,
                                        iterations=3, dim=3, locally_dense=False,
                               em_routing=False)
        self.caps1 = CapsLayer(num_caps_in=6, num_caps_out=6,
                                        channels_in=4, channels_out=4,
                                        iterations=3, dim=3, locally_dense=True,
                               em_routing=False)
        self.pool1 = PoolingByAgreement(num_caps_in=6, num_caps_out=6,
                                        channels_in=4, channels_out=4,
                                        iterations=3, dim=3)
        self.caps3 = CapsLayer(num_caps_in=6, num_caps_out=8,
                                        channels_in=4, channels_out=4,
                                        iterations=3, dim=3, locally_dense=True,
                               em_routing=False)
        self.pool3 = PoolingByAgreement(num_caps_in=8, num_caps_out=8,
                                        channels_in=4, channels_out=4,
                                        iterations=3, dim=3)
        self.caps5 = CapsLayer(num_caps_in=8, num_caps_out=12,
                                        channels_in=4, channels_out=4,
                                        iterations=3, dim=3, locally_dense=True,
                               em_routing=False)
        self.pool5 = PoolingByAgreement(num_caps_in=12,
                                        num_caps_out=12,
                                        channels_in=4, channels_out=4,
                                        iterations=3, dim=3)
        self.caps6 = CapsLayer(num_caps_in=12, num_caps_out=14,
                                        channels_in=4, channels_out=4,
                                        iterations=3, dim=3, locally_dense=True,
                               em_routing=False)
        self.pool6 = PoolingByAgreement(num_caps_in=14, num_caps_out=14,
                                        channels_in=4, channels_out=4,
                                        iterations=3, dim=3)
        self.caps7 = CapsLayer(num_caps_in=14, num_caps_out=num_classes,
                                        channels_in=4, channels_out=4,
                                        iterations=3, dim=3, locally_dense=True,
                               em_routing=False)
    ...
    def forward(self, data):
        #pose = F.elu(self.conv1(data.adj, data.input))
        #data.pose_vectors = self.build_pose_vectors(6, input=pose)
        data.pose_vectors = self.build_pose_vectors(1, size=data.input.size(0))
        data.input = data.input.view(-1, 1)#.expand(-1, 6).contiguous()
